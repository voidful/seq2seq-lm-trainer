...
def process_and_generate(input_unit, cs):
    inputs = tokenizer(
        "".join([f"v_tok_{i}" for i in input_unit]), return_tensors="pt").to('cuda')
    code = tokenizer.batch_decode(model.generate(**inputs, max_length=1024))[0]
    code = [int(i) for i in code.replace(
        "</s>", "").replace("<s>", "").split("v_tok_")[1:]]
    audio = cs(code)
    input_features = whisper_processor(audio, sampling_rate=16000, return_tensors="pt")[
...
for qa_item in dataset['dev']:
    ...
    ans_dict = {}
    try:
        question_unit = json.loads(qa_item['hubert_100_question_unit'])[
            0]["merged_code"]
        context_unit = json.loads(qa_item['hubert_100_context_unit'])[
            0]["merged_code"]
        answer_unit = json.loads(qa_item['hubert_100_answer_unit'])[
            0]["merged_code"]
    except:
        count += 1
        continue
    ...
    input_features = whisper_processor(
        groundtruth_answer, sampling_rate=16000, return_tensors="pt")['input_features'].to('cuda')
    predicted_ids = whisper_model.generate(input_features)
...
for qa_item in dataset['dev']:
    ...
    ans_dict = {}
    try:
        question_unit = json.loads(qa_item['mhubert_1000_question_unit'])[
            0]["merged_code"]
        context_unit = json.loads(qa_item['mhubert_1000_context_unit'])[
            0]["merged_code"]
        answer_unit = json.loads(qa_item['mhubert_1000_answer_unit'])[
            0]["merged_code"]
    except:
        continue
    groundtruth_answer = cs(answer_unit)
