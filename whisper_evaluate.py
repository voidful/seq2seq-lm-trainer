...
whisper_model = WhisperForConditionalGeneration.from_pretrained(
    "Oscarshih/Whisper-small-NMSQA"
).to("cuda")
whisper_model.config.forced_decoder_ids = None

tokenizer = AutoTokenizer.from_pretrained("Oscarshih/long-t5-base-SQA-15ep")
model = AutoModelForSeq2SeqLM.from_pretrained("Oscarshih/long-t5-base-SQA-15ep").to(
    "cuda"
)
...
def process_and_generate(input_unit, cs):
    inputs = tokenizer(
        "".join([f"v_tok_{i}" for i in input_unit]), return_tensors="pt"
    ).to("cuda")
    code = tokenizer.batch_decode(model.generate(**inputs, max_length=1024))[0]
    code = [
        int(i) for i in code.replace("</s>", "").replace("<s>", "").split("v_tok_")[1:]
    ]
    audio = cs(code)
...
for qa_item in dataset["dev"]:
    ...
    ans_dict = {}
    try:
        question_unit = json.loads(qa_item["hubert_100_question_unit"])[0][
            "merged_code"
        ]
    ...
    input_features = whisper_processor(
        groundtruth_answer, sampling_rate=16000, return_tensors="pt"
...
for qa_item in dataset["dev"]:
    ...
    ans_dict = {}
    try:
        question_unit = json.loads(qa_item["mhubert_1000_question_unit"])[0][
            "merged_code"
        ]
        context_unit = json.loads(qa_item["mhubert_1000_context_unit"])[0][
            "merged_code"
        ]
        answer_unit = json.loads(qa_item["mhubert_1000_answer_unit"])[0]["merged_code"]
    except:
        continue
    groundtruth_answer = cs(answer_unit)
    input_features = whisper_processor(
